Velero Backup - Opt-out Annotation
    - All file system backups (FSB) will be taken in the specified namespace.
    - FSB happens via Kopia.
    - To exclude specific volumes from being backed up, use the backup.velero.io/backup-volumes-excludes annotation on the pod.
...
#velero backup create fs-backup-optout-annotate --include-namespaces gcp --include-resources deployments,pods,persistentvolumeclaims
velero backup create fs-backup-optout-annotate --include-namespaces gcp

#Verify backups
velero backup get
velero backup describe fs-backup-optout-annotate
